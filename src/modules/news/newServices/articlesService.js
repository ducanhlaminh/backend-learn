            ...
            getDetailService: async (slug, slug_crc) => {
                ...
                if (checkslugsrc !== null) {
                    ...
                    if (article !== null) {
                        ...
                        const category = await db.new_articles_category.findAll(
                            {
                                where: {
        ...
        get: {
            getAllService: async ({
                ...
                try {
                    ...
                    if (user.role === 2) {
                        whereCondition.created_user_id = user.id;
                        console.log(whereCondition, "log");
                    } else {
            ...
            getHotService: async ({ ...query }) => {
                try {
                    ...
                    if (query.slug_crc === "1") {
                        ...
                        const result = await db.new_category.findOne({
                            where: {
                                slug_crc: query.slug_crc,
                            },
                            order: [
                                [
                        ...
                        hot_news = result.new_articles_hot_categories;
                    }
        ...
        create: {
            createHotMain: async (data) => {
                if (!data.category_id) {
                    ...
                    await db.new_articles_hot_main.bulkCreate(data.articles);
                } else {
                    await db.new_articles_hot_category.destroy({
                ...
                try {
                    ...
                    for (let cate of cates) {
                        ...
                        for (let subCate of cate.sub) {
                            const slug_crc_subCate = crc32(subCate.slug);
                    ...
                    async function downloadImage(object, name, pathDir) {
                        ...
                        try {
                            const response = await axios.get(object.avatar, {
                    ...
                    for (let articel of articels) {
                        ...
                        for (let i = 0; i < articel.article.length; i++) {
                            var newArticle;
                            const slug_crc = crc32(articel.article[i].slug);
                            try {
                                // await downloadImage(
                                //         articel.article[
                                //                 i
                            ...
                            if (i < 9) {
                                ...
                                subCate.childCategories.length >=
                                Math.floor(i / 9)
                            ) {
                                console.log((i % 9) + 1);
                                await db.new_articles_category.create({
